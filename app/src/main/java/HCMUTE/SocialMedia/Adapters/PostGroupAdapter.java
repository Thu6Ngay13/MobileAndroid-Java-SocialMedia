...
import HCMUTE.SocialMedia.Holders.PostGroupHolder;
...
public class PostGroupAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public PostGroupAdapter(Context context, List<PostCardModel> posts) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == TypeViewLoad.VIEW_TYPE_ITEM.ordinal()) {
            return new PostGroupHolder(LayoutInflater.from(context).inflate(R.layout.post_group_view, parent, false));
        } else {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof PostGroupHolder) {
            PostGroupHolder postGroupHolder = (PostGroupHolder) holder;
            PostCardModel postCardModel = posts.get(position);
            postGroupHolder.fullName.setText(postCardModel.getFullName());
            Log.d("TAG",  "///"+String.valueOf(postCardModel.getGroupId()));


            apiService = RetrofitClient.getRetrofit().create(APIService.class);
            apiService.findOne(postCardModel.getGroupId()).enqueue(new Callback<SimpleResponse<GroupModel>>() {
                @Override
                public void onResponse(Call<SimpleResponse<GroupModel>> call, Response<SimpleResponse<GroupModel>> response) {
                    if (response.isSuccessful()) {
                        ...
                        if (simpleResponse.getResult() != null) {
                            ...
                            GroupModel groupModel = simpleResponse.getResult();
                            postGroupHolder.groupName.setText(groupModel.getGroupName());
                            Glide.with(context)
                                    .load(groupModel.getAvatarURL())
                                    .into(postGroupHolder.groupAvatar);
            ...
            postGroupHolder.postingTimeAt.setText(timeShow);
            postGroupHolder.postText.setText(postCardModel.getPostText());


            Glide.with(context)
                    .load(postCardModel.getPostMedia())
                    .into(postGroupHolder.postImage);
            if (postCardModel.getMode() == 1){
                postGroupHolder.mode.setImageResource(R.mipmap.ic_global_72_dark);
            } else if (postCardModel.getMode() == 2) {
                postGroupHolder.mode.setImageResource(R.mipmap.ic_friend_72_full);
            } else if (postCardModel.getMode() == 3) {
                postGroupHolder.mode.setImageResource(R.mipmap.ic_lock_72_dark);
            }
            else {
                postGroupHolder.mode.setImageResource(R.mipmap.ic_user_groups_72_full);
            }
    ...
    public void addItems(List<PostCardModel> postCardModels, RecyclerView recyclerView) {
        ...
        PostGroupAdapter.AsyncTaskUI asyncTaskUI = new PostGroupAdapter.AsyncTaskUI(postCardModels, recyclerView);
        asyncTaskUI.execute();
    }
